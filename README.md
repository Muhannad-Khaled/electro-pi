...
- **Sections 1–2 use managed Gemini APIs.** Tool calling and relevance
  ...
  `gemini-3.1-flash-lite`; Section 2 uses the same model (chosen after
  free-tier quota changes retired the originally planned `gemini-2.0-flash`
  — details in section2_rag/README.md).
...
`get_flight_status(flight_id)` with a mocked lookup, and the committed
